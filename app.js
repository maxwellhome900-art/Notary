...
const DEFAULT_CANDIDATE_NAME = 'Yuliya';
const CANDIDATE_NAME_STORAGE_KEY = 'notaryCandidateName';
const EXAM_SIZE = 40;
...
const examChooser = document.getElementById('exam-chooser');
...
function init(year) {
  ...
  questions = shuffled.slice(0, Math.min(EXAM_SIZE, shuffled.length));
  userAnswers = [];
  answered = new Set();
  currentIndex = 0;
...
function showResults(isEarlyExit = false) {
  ...
  examYearEl.textContent = `${candidateName} — ${selectedYear} Practice Exam${isEarlyExit ? ' (Exited early)' : ''}`;
  ...
  if (total === 0) {
    ...
    msgEl.textContent = `Excellent work, ${candidateName}! You demonstrate strong knowledge of NYS notary requirements.`;
  } else if (pct >= 70) {
    ...
    msgEl.textContent = `Good effort, ${candidateName}! Review the explanations and try again to strengthen your knowledge.`;
  } else {
    ...
    msgEl.textContent = `Keep studying, ${candidateName}! Review the NYS Department of State notary materials and try again.`;
  }
...
function buildFlashcardOrder() {
  // Use all available questions for flashcards
  flashcardOrder = shuffleArray(QUESTIONS, Date.now() % 1000000);
  flashcardIndex = 0;
}
...
if (startExamBtn && examChooser) {
  startExamBtn.addEventListener('click', () => {
    if (!ensureCandidateNameFromInput()) return;
    examChooser.classList.remove('hidden');
    examChooser.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
}
